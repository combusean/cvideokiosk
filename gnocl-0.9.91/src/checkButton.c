...
int gnoclCheckButtonCmd( ClientData data, Tcl_Interp *interp,
      int objc, Tcl_Obj * const objv[] )
{
   GnoclCheckParams *para;
   int ret;
   
   if( gnoclParseOptions( interp, objc, objv, checkButtonOptions ) 
         != TCL_OK )
   {
      gnoclClearOptions( checkButtonOptions );
      return TCL_ERROR;
   ...
   ret = gnoclSetOptions( interp, checkButtonOptions, 
