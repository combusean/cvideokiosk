      ...
      int objc, Tcl_Obj * const objv[] )
{
   const char *cmds[] = { "delete", "configure", 
         "add", "addRow", "addColumn", NULL };
   enum cmdIdx { DeleteIdx, ConfigureIdx, 
         AddIdx, AddRowIdx, AddColumnIdx };
      ...
      int objc, Tcl_Obj * const objv[] )
{
   ...
   /* set default values */
   gtk_table_set_row_spacings( table, GNOCL_PAD );
   gtk_table_set_col_spacings( table, GNOCL_PAD );
   gtk_container_set_border_width( GTK_CONTAINER( table ), GNOCL_PAD );
