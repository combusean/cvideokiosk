...
static const int fontSelectIdx = 6; /* first index which relates to the
                                       GtkFontSelection and not to the dialog */
typedef struct
{
   char             *name;
   Tcl_Interp       *interp;
...
static void onButtonFunc( FontSelParams *para, int isOk )
{
   if( para->onClicked )
   {
     GnoclPercSubst ps[] = {
...
{
   onButtonFunc( (FontSelParams *)data, 1 );
}

static void onCancelFunc( GtkWidget *widget, gpointer data )
{
   onButtonFunc( (FontSelParams *)data, 0 );
}

static int fontSelFunc( ClientData data, Tcl_Interp *interp,
      int objc, Tcl_Obj * const objv[] )
{
   static const char *cmds[] = { "delete", "configure", NULL };
   enum cmdIdx { DeleteIdx, ConfigureIdx };
   FontSelParams *para = (FontSelParams *)data;
      ...
      int objc, Tcl_Obj * const objv[] )
{
   ...
   else
   {
      ...
      Tcl_CreateObjCommand( interp, para->name, fontSelFunc, para, NULL );
